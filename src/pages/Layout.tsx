  ...
  return (
    <div className={`min-h-screen relative overflow-hidden transition-colors duration-300 ${darkMode ? 'bg-[#06060a] text-slate-100' : 'bg-slate-50 text-slate-900'}`}>
      ...
      <header className="sticky top-0 z-50 w-full px-4 py-4 md:px-8 max-w-7xl mx-auto">
        <div className="glass rounded-2xl px-6 py-4 flex items-center justify-between shadow-sm relative">

          <Link to="/" className="flex items-center gap-2 no-underline">
            <div className="w-10 h-10 rounded-xl bg-black flex items-center justify-center font-bold text-white shadow-sm glow-primary">
              <span className="text-xl">⚡</span>
            </div>
            <div>
              <span className="text-lg font-bold tracking-tight text-black">FreeUI</span>
              <span className="text-[10px] block font-mono text-slate-500 -mt-1 font-bold">V1.2.0 OPEN-SOURCE</span>
            </div>
          </Link>
            ...
            <div className="flex items-center gap-1.5 px-2 py-1 rounded-xl bg-slate-100 border border-slate-200">
              {FRAMEWORKS.map(fw => (
                <button
                  ...
                  className={`px-3 py-1 rounded-lg text-xs font-semibold tracking-wide transition-all cursor-pointer ${globalTab === fw.id ? 'bg-black text-white shadow-sm' : 'text-slate-500 hover:text-black hover:bg-slate-200'}`}
                >
          ...
          <div className="flex items-center gap-3">
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="hidden sm:flex items-center gap-2 px-4 py-2.5 rounded-xl border border-slate-200 hover:bg-slate-50 text-slate-700 hover:text-black transition-all text-xs font-semibold tracking-wide"
            >
            ...
            <button
              ...
              className="px-5 py-2.5 rounded-xl bg-black hover:bg-slate-800 text-white font-semibold text-xs tracking-wide active:scale-95 transition-all duration-300 cursor-pointer shadow-sm"
            >
      ...
      <footer className="relative z-10 border-t border-slate-200 bg-white py-8 text-center text-xs text-slate-500 font-semibold tracking-wide">
        <div className="max-w-7xl mx-auto px-4 flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-1.5 text-slate-800">
            <span className="text-black font-bold">⚡</span>
            <span>FreeUI - Minimalist Component Portal.</span>
          </div>
